# scripts/_lib.sh — funciones compartidas por actualizar_menu.sh y actualizar_site.sh
# (cronometraje de pasos, recolección de valoraciones descargadas, el paso de
# `git pull` inicial que ambos ejecutan primero, y la tabla resumen final).
# Se carga con `source`, nunca se ejecuta directamente.
...
# Paso "git pull" que ambos scripts ejecutan primero — trae los ratings que el
# Worker de Cloudflare sincronizó al repo desde el sitio.
# $1 = etiqueta de progreso a imprimir, p.ej. "[1/6]".
_paso_git_pull() {
  local label="$1"
  T_STEP=$(_now_s)
  echo "▶ $label Trayendo cambios de GitHub (ratings sincronizados desde el sitio)..."
...
# Tabla resumen final con tiempos por paso, hasta el borde inferior inclusive.
# $1 = título de la sección (p.ej. "Sitio listo — resumen de ejecución").
# Requiere T_GLOBAL ya inicializado por el llamador. El llamador imprime
# cualquier contenido adicional propio y el borde de cierre después de esto.
_imprimir_resumen_final() {
  local titulo="$1"
  local total_elapsed
  total_elapsed=$(_elapsed "$T_GLOBAL")
